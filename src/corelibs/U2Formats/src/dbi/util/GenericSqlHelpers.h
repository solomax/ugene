...
#ifndef _U2_GENERIC_SQL_HELPERS_H_
#define _U2_GENERIC_SQL_HELPERS_H_
...
class U2FORMATS_EXPORT GenericSqlDbRef {
public:
                                    GenericSqlDbRef();

    QSqlDatabase                    handle;
    QMutex                          mutex;
    QVector<GenericSqlTransaction *>     transactionStack;
};

class U2FORMATS_EXPORT GenericSqlUtils {
public:
    /**
        Removes from the table all records with 'field' == id
        Checks 'expectedRowCount' the same way as 'update' method
    */
    static qint64 remove(const QString& table, const QString& field, const U2DataId& id, qint64 expectedRows, GenericSqlDbRef* db, U2OpStatus& os);

    /** Checks if the table exists in database */
    static bool isTableExists(const QString& tableName, GenericSqlDbRef* db, U2OpStatus& os);
};
...
public:
    ...
    */
    U2SqlQuery(const QString& sql, GenericSqlDbRef* db, U2OpStatus& os);
    U2SqlQuery(const QString& sql, qint64 offset, qint64 count, GenericSqlDbRef* db, U2OpStatus& os);
    ~U2SqlQuery();
    ...
    */
    ...
    const GenericSqlDbRef* getDb() const;
...
class GenericSqlTransaction {
public:
    GenericSqlTransaction(GenericSqlDbRef *db, U2OpStatus &os);
...
template <class T> class GenericSqlRSLoader {
public:
    virtual ~GenericSqlRSLoader(){}
    virtual T load(U2SqlQuery* q) = 0;
};

/** Filter for SqlRSIterator. Checks if value must be filtered out from the result */
template <class T> class GenericSqlRSFilter {
public:
    virtual ~GenericSqlRSFilter(){}
    virtual bool filter(const T&) = 0;
};

/** SQL query result set iterator */
template<class T> class GenericSqlRSIterator : public U2DbiIterator<T> {
public:
    GenericSqlRSIterator(QSharedPointer<U2SqlQuery> q, GenericSqlRSLoader<T>* l, GenericSqlRSFilter<T>* f, const T& d, U2OpStatus& o)
        : query(q), loader(l), filter(f), defaultValue(d), os(o), endOfStream(false)
    {
        fetchNext();
    }

    virtual ~GenericSqlRSIterator() {
        delete filter;
        delete loader;
        query.clear();
    }
...
private:
    ...
    GenericSqlRSLoader<T>* loader;
    GenericSqlRSFilter<T>* filter;
    T               defaultValue;
    U2OpStatus&     os;
    bool            endOfStream;
    T               nextResult;
...
class GenericSqlDataIdRSLoader : public GenericSqlRSLoader<U2DataId> {
public:
    GenericSqlDataIdRSLoader(U2DataType _type, const QByteArray& _dbExra = QByteArray()) : type(_type), dbExtra(_dbExra){}
...
class GenericSqlDataIdRSLoaderEx : public GenericSqlRSLoader<U2DataId> {
public:
    GenericSqlDataIdRSLoaderEx(const QByteArray& _dbExra = QByteArray()) : dbExtra(_dbExra){}
...
#endif // _U2_GENERIC_SQL_HELPERS_H_
