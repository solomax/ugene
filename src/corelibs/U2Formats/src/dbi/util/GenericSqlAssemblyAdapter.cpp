...
GenericSqlAssemblyAdapter::GenericSqlAssemblyAdapter(const U2DataId& assemblyId,
...
GenericSqlAssemblyNameFilter::GenericSqlAssemblyNameFilter(const QByteArray& expectedName) :
    name (expectedName)
{
}

bool GenericSqlAssemblyNameFilter::filter(const U2AssemblyRead& r) {
    return name == r->name;
}

U2AssemblyRead GenericSqlSimpleAssemblyReadLoader::load(U2SqlQuery* q) {
    ...
    GenericSqlAssemblyUtils::unpackData(data, read, q->getOpStatus());
    if (q->hasError()) {
        return U2AssemblyRead();
    }
...
PackAlgorithmData GenericSqlSimpleAssemblyReadPackedDataLoader::load(U2SqlQuery* q) {
