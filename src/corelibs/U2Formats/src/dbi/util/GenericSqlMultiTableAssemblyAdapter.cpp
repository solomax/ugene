...
#include "GenericSqlMultiTableAssemblyAdapter.h"
#include "GenericSqlSingleTableAssemblyAdapter.h"
#include "dbi/GenericSqlDbi.h"
#include "dbi/GenericSqlAssemblyDbi.h"
...
/****************************************************************/
/* GenericSqlMtaSingleTableAdapter */
/****************************************************************/

GenericSqlMtaSingleTableAdapter::GenericSqlMtaSingleTableAdapter(GenericSqlSingleTableAssemblyAdapter* adapter,
                                                       int rowPos,
                                                       int elenPos,
...
/****************************************************************/
/* GenericSqlMultiTableAssemblyAdapter */
/****************************************************************/

GenericSqlMultiTableAssemblyAdapter::GenericSqlMultiTableAssemblyAdapter(GenericSqlDbi* dbi,
...
GenericSqlMultiTableAssemblyAdapter::~GenericSqlMultiTableAssemblyAdapter() {
    clearTableAdaptersInfo();
}

qint64 GenericSqlMultiTableAssemblyAdapter::countReads(const U2Region& r, U2OpStatus& os) {
    ...
    foreach(GenericSqlMtaSingleTableAdapter* a, adapters) {
...
qint64 GenericSqlMultiTableAssemblyAdapter::getMaxPackedRow(const U2Region& r, U2OpStatus& os) {
    qint64 max = 0;

    // process only hi row adapters
    int nRows = adaptersGrid.size();
    for (int rowPos = nRows; --rowPos >= 0 && max == 0;) {
        QVector<GenericSqlMtaSingleTableAdapter*> elenAdapters = adaptersGrid.at(rowPos);
        for (int elenPos = 0, nElens = elenAdapters.size(); elenPos < nElens; elenPos++) {
            GenericSqlMtaSingleTableAdapter* a = elenAdapters.at(elenPos);
            if (a == NULL) {
                continue;
            }
...
qint64 GenericSqlMultiTableAssemblyAdapter::getMaxEndPos(U2OpStatus& os) {
...
U2DbiIterator<U2AssemblyRead>* GenericSqlMultiTableAssemblyAdapter::getReads(const U2Region& r, U2OpStatus& os, bool sortedHint) {
    ...
    return new GenericSqlMtaReadsIterator(iterators, idExtras, sortedHint);
}

U2DbiIterator<U2AssemblyRead>* GenericSqlMultiTableAssemblyAdapter::getReadsByRow(const U2Region& r, qint64 minRow, qint64 maxRow, U2OpStatus& os) {
    QVector< U2DbiIterator<U2AssemblyRead>* > iterators;
    ...
    return new GenericSqlMtaReadsIterator(iterators, selectedIdExtras, false);
}

U2DbiIterator<U2AssemblyRead>* GenericSqlMultiTableAssemblyAdapter::getReadsByName(const QByteArray& name, U2OpStatus& os) {
    QVector< U2DbiIterator<U2AssemblyRead>* > iterators;
    ...
    return new GenericSqlMtaReadsIterator(iterators, idExtras, false);
}

void GenericSqlMultiTableAssemblyAdapter::addReads(U2DbiIterator<U2AssemblyRead>* it, U2AssemblyReadsImportInfo& ii, U2OpStatus& os) {
        ...
        if (lastIteration || readsInGrid > N_READS_TO_FLUSH_TOTAL) {
            for (int rowPos = 0; rowPos < nRows && !os.isCoR(); rowPos++) {
                for (int elenPos = 0; elenPos < nElens && !os.isCoR(); elenPos++) {
                    ...
                    GenericSqlMtaSingleTableAdapter* adapter = getAdapterByRowAndElenRange(rowPos, elenPos, true, os);
...
void GenericSqlMultiTableAssemblyAdapter::removeReads(const QList<U2DataId>& readIds, U2OpStatus& os) {
    int nReads = readIds.size();

    QHash<GenericSqlMtaSingleTableAdapter*, QList<U2DataId> > readsByAdapter;
    for (int i = 0; i < nReads; i++) {
        const U2DataId& readId = readIds[i];
        int rowPos = getRowRangePosById(readId);
        int elenPos = getElenRangePosById(readId);
        GenericSqlMtaSingleTableAdapter* a = getAdapterByRowAndElenRange(rowPos, elenPos, false, os);
    ...
    foreach(GenericSqlMtaSingleTableAdapter* a, readsByAdapter.keys()) {
...
void GenericSqlMultiTableAssemblyAdapter::dropReadsTables(U2OpStatus &os) {
    foreach (QVector<GenericSqlMtaSingleTableAdapter*> adaptersVector, adaptersGrid) {
        foreach (GenericSqlMtaSingleTableAdapter* adapter, adaptersVector) {
            if (NULL != adapter) {
...
void GenericSqlMultiTableAssemblyAdapter::pack(U2AssemblyPackStat& stat, U2OpStatus& os) {
    GenericSqlMultiTablePackAlgorithmAdapter packAdapter(this);
...
void GenericSqlMultiTableAssemblyAdapter::calculateCoverage(const U2Region& region, U2AssemblyCoverageStat& coverage, U2OpStatus& os) {
    for (int i = 0; i < adapters.size(); ++i) {
        GenericSqlMtaSingleTableAdapter * a = adapters.at(i);
...
void GenericSqlMultiTableAssemblyAdapter::createReadsIndexes(U2OpStatus& os) {
    foreach(GenericSqlMtaSingleTableAdapter* a, adapters) {
...
int GenericSqlMultiTableAssemblyAdapter::getElenRangePosByLength(qint64 readLength) const {
...
int GenericSqlMultiTableAssemblyAdapter::getElenRangePosById(const U2DataId& id) const {
...
int GenericSqlMultiTableAssemblyAdapter::getNumberOfElenRanges() const {
    return elenRanges.size();
}

int GenericSqlMultiTableAssemblyAdapter::getRowRangePosByRow(quint64 row) const {
    return row / rowsPerRange;
}

int GenericSqlMultiTableAssemblyAdapter::getRowRangePosById(const U2DataId& id) const {
    QByteArray extra = U2DbiUtils::toDbExtra(id);
...
int GenericSqlMultiTableAssemblyAdapter::getRowsPerRange() const {
    return rowsPerRange;
}

const QVector<GenericSqlMtaSingleTableAdapter*>& GenericSqlMultiTableAssemblyAdapter::getAdapters() const {
    return adapters;
}
const QVector<QByteArray>& GenericSqlMultiTableAssemblyAdapter::getIdExtrasPerRange() const {
    return idExtras;
}

GenericSqlDbRef* GenericSqlMultiTableAssemblyAdapter::getDbRef() const {
    return dbi->getDbRef();
}

GenericSqlMtaSingleTableAdapter* GenericSqlMultiTableAssemblyAdapter::getAdapterByRowAndElenRange(int rowPos, int elenPos, bool createIfNotExits, U2OpStatus& os) {
    int nElens = elenRanges.size();
    ...
    GenericSqlMtaSingleTableAdapter* adapter = elenAdapters.at(elenPos);
...
QString GenericSqlMultiTableAssemblyAdapter::getTableSuffix(int rowPos, int elenPos) {
...
QByteArray GenericSqlMultiTableAssemblyAdapter::getIdExtra(int rowPos, int elenPos) {
    QByteArray res(4, 0);
...
void GenericSqlMultiTableAssemblyAdapter::syncTables(U2OpStatus& os) {
...
void GenericSqlMultiTableAssemblyAdapter::initTables(const QList<U2AssemblyRead>& reads, U2OpStatus& os) {
...
void GenericSqlMultiTableAssemblyAdapter::rereadTables(const QByteArray& idata, U2OpStatus& os) {
    ...
    for (int rowPos = 0; rowPos < nRows; rowPos++) {
        for (int elenPos = 0; elenPos < nElens; elenPos++) {
            ...
            const QString tableName = GenericSqlSingleTableAssemblyAdapter::getReadsTableName(assemblyId, 'M', suffix);
            if (GenericSqlUtils::isTableExists(tableName, db, os)) {
...
void GenericSqlMultiTableAssemblyAdapter::flushTables(U2OpStatus& os) {
...
void GenericSqlMultiTableAssemblyAdapter::clearTableAdaptersInfo() {
    qDeleteAll(adapters);
    adaptersGrid.clear();
    idExtras.clear();
    elenRanges.clear();
...
void GenericSqlMultiTableAssemblyAdapter::initAdaptersGrid(int nRows, int nElens) {
    SAFE_POINT(adaptersGrid.isEmpty(), "Adapters are already initialized", );

    adaptersGrid.resize(nRows);
    for (int i = 0; i < nRows; i++) {
        adaptersGrid[i] = QVector<GenericSqlMtaSingleTableAdapter*>(nElens, NULL);
    }
}


//////////////////////////////////////////////////////////////////////////
// GenericSqlReadTableMigrationData

GenericSqlReadTableMigrationData::GenericSqlReadTableMigrationData() :
    readId(-1),
    oldTable(NULL),
    newProw(-1)
{
...
GenericSqlReadTableMigrationData::GenericSqlReadTableMigrationData(qint64 oldId, GenericSqlMtaSingleTableAdapter* oldT, int newP) :
    readId(oldId),
    oldTable(oldT),
    newProw(newP)
{
...
GenericSqlMultiTablePackAlgorithmAdapter::GenericSqlMultiTablePackAlgorithmAdapter(GenericSqlMultiTableAssemblyAdapter* ma) :
    multiTableAdapter(ma)
{
    GenericSqlDbRef* db = multiTableAdapter->getDbRef();
    int nElens = multiTableAdapter->getNumberOfElenRanges();
    ensureGridSize(nElens);

    foreach (GenericSqlMtaSingleTableAdapter* a, multiTableAdapter->getAdapters()) {
        GenericSqlSingleTablePackAlgorithmAdapter* sa = new GenericSqlSingleTablePackAlgorithmAdapter(db, a->singleTableAdapter->getReadsTableName());
        packAdapters << sa;
...
GenericSqlMultiTablePackAlgorithmAdapter::~GenericSqlMultiTablePackAlgorithmAdapter() {
    qDeleteAll(packAdapters);
}

U2DbiIterator<PackAlgorithmData>* GenericSqlMultiTablePackAlgorithmAdapter::selectAllReads(U2OpStatus& os) {
    QVector< U2DbiIterator<PackAlgorithmData>* > iterators;
    foreach (GenericSqlSingleTablePackAlgorithmAdapter* a, packAdapters) {
        iterators << a->selectAllReads(os);
    }

    return new GenericSqlMTAPackAlgorithmDataIterator(iterators, multiTableAdapter->getIdExtrasPerRange());
}

void GenericSqlMultiTablePackAlgorithmAdapter::assignProw(const U2DataId& readId, qint64 prow, U2OpStatus& os) {
    ...
    GenericSqlSingleTablePackAlgorithmAdapter* sa = NULL;
    ...
    GenericSqlMtaSingleTableAdapter* oldA = multiTableAdapter->getAdapterByRowAndElenRange(oldRowPos, elenPos, false, os);
    GenericSqlMtaSingleTableAdapter* newA = multiTableAdapter->getAdapterByRowAndElenRange(newRowPos, elenPos, true, os);
    ...
    if (sa == NULL) {
        sa = new GenericSqlSingleTablePackAlgorithmAdapter(multiTableAdapter->getDbRef(), newA->singleTableAdapter->getReadsTableName());
        packAdapters << sa;
        packAdaptersGrid[newRowPos][elenPos] = sa;
    }

    QVector<GenericSqlReadTableMigrationData>& newTableData = migrations[newA];
    newTableData.append(GenericSqlReadTableMigrationData(U2DbiUtils::toDbiId(readId), oldA, prow));
    //TODO: add mem check here!
}

void GenericSqlMultiTablePackAlgorithmAdapter::releaseDbResources() {
    foreach (GenericSqlSingleTablePackAlgorithmAdapter* a, packAdapters) {
        a->releaseDbResources();
    }
}

void GenericSqlMultiTablePackAlgorithmAdapter::migrateAll(U2OpStatus& os) {
    qint64 nReadsToMigrate = 0;
    foreach (GenericSqlMtaSingleTableAdapter* newTable, migrations.keys()) {
        const QVector<GenericSqlReadTableMigrationData>& data = migrations[newTable];
    ...
    if (migrationPercent > MAX_PERCENT_TO_REINDEX) {
        ...
        foreach (GenericSqlMtaSingleTableAdapter* adapter, multiTableAdapter->getAdapters()) {
...
void GenericSqlMultiTablePackAlgorithmAdapter::ensureGridSize(int nRows) {
...
void GenericSqlMultiTablePackAlgorithmAdapter::migrate(GenericSqlMtaSingleTableAdapter* newA, const QVector<GenericSqlReadTableMigrationData>& data, qint64 migratedBefore, qint64 totalMigrationCount, U2OpStatus& os) {
    //delete reads from old tables, and insert into new one
    QHash<GenericSqlMtaSingleTableAdapter*, QVector<GenericSqlReadTableMigrationData> > readsByOldTable;
    foreach (const GenericSqlReadTableMigrationData& d, data) {
    ...
    foreach (GenericSqlMtaSingleTableAdapter* oldA, readsByOldTable.keys()) {

        const QVector<GenericSqlReadTableMigrationData>& migData = readsByOldTable[oldA];
        if (migData.isEmpty()) {
            continue;
        }
        ...
        { //nested block is needed to ensure all queries are finalized
            ...
            foreach(const GenericSqlReadTableMigrationData& d, migData) {
...
GenericSqlMtaReadsIterator::GenericSqlMtaReadsIterator(QVector< U2DbiIterator<U2AssemblyRead>* >& i, const QVector<QByteArray>& ie, bool sorted)
: iterators (i), currentRange(0), idExtras(ie), sortedHint(sorted)
{
}

GenericSqlMtaReadsIterator::~GenericSqlMtaReadsIterator() {
    qDeleteAll(iterators);
}

// TODO: remove copy-paste from this code
bool GenericSqlMtaReadsIterator::hasNext() {
    if (sortedHint) {
...
U2AssemblyRead GenericSqlMtaReadsIterator::next() {
    U2AssemblyRead res;
    if (sortedHint) {
...
U2AssemblyRead GenericSqlMtaReadsIterator::peek() {
    U2AssemblyRead res;
    if (sortedHint) {
        qint64 minPos = LLONG_MAX;
        U2DbiIterator<U2AssemblyRead> *minIt = NULL;
...
GenericSqlMTAPackAlgorithmDataIterator::GenericSqlMTAPackAlgorithmDataIterator(QVector< U2DbiIterator<PackAlgorithmData>* >& i, const QVector<QByteArray>& ie)
:  iterators (i), idExtras(ie)
{
    fetchNextData();
}

GenericSqlMTAPackAlgorithmDataIterator::~GenericSqlMTAPackAlgorithmDataIterator() {
    qDeleteAll(iterators);
}

bool GenericSqlMTAPackAlgorithmDataIterator::hasNext() {
    return !nextData.readId.isEmpty();
}

PackAlgorithmData GenericSqlMTAPackAlgorithmDataIterator::next() {
    PackAlgorithmData res = nextData;
    fetchNextData();
    return res;
}

PackAlgorithmData GenericSqlMTAPackAlgorithmDataIterator::peek() {
    return nextData;
}

void GenericSqlMTAPackAlgorithmDataIterator::fetchNextData() {
