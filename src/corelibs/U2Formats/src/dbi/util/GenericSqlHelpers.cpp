...
GenericSqlDbRef::GenericSqlDbRef() :
    mutex(QMutex::Recursive)
{
}

qint64 GenericSqlUtils::remove(const QString& table, const QString& field, const U2DataId& id, qint64 expectedRows, GenericSqlDbRef* db, U2OpStatus& os) {
...
bool GenericSqlUtils::isTableExists(const QString& tableName, GenericSqlDbRef* db, U2OpStatus& os) {
...
U2SqlQuery::U2SqlQuery(const QString& sql, GenericSqlDbRef* db, U2OpStatus& _os) :
    db(db),
    os(&_os),
...
U2SqlQuery::U2SqlQuery(const QString& sql, qint64 offset, qint64 count, GenericSqlDbRef* db, U2OpStatus& _os) :
    db(db),
    os(&_os),
    query(NULL == db ? QSqlDatabase() : db->handle)
{
    ...
    U2DbiUtils::addLimit(sqlString, offset, count);  throw 0; //FIXME TODO, this is DB specific
    query.setForwardOnly(true);
...
const GenericSqlDbRef* U2SqlQuery::getDb() const {
    return db;
}
...
GenericSqlTransaction::GenericSqlTransaction(GenericSqlDbRef* db, U2OpStatus& os) :
    db(db),
    os(os),
    started(false)
{
...
GenericSqlTransaction::~GenericSqlTransaction() {
    QMutexLocker locker(&db->mutex);
    CHECK(started, );
