...
#ifndef _U2_GENERIC_SQL_ASSEMBLY_MULTI_TABLE_DBI_H_
#define _U2_GENERIC_SQL_ASSEMBLY_MULTI_TABLE_DBI_H_

#include <QtCore/QReadWriteLock>

#include <U2Core/U2SqlHelpers.h>

#include "GenericSqlAssemblyAdapter.h"
#include "util/AssemblyPackAlgorithm.h"

namespace U2 {

class GenericSqlDbi;
class GenericSqlSingleTableAssemblyAdapter;
class GenericSqlSingleTablePackAlgorithmAdapter;

class GenericSqlMtaSingleTableAdapter {
public:
    /** Wrapper over 1 table in database. If singleTableAdapter == NULL the table was not created yet */
    GenericSqlMtaSingleTableAdapter(GenericSqlSingleTableAssemblyAdapter* adapter, int rowPos, int elenPos, const QByteArray &extra);

    GenericSqlSingleTableAssemblyAdapter* singleTableAdapter;
    int                         rowPos;
    int                         elenPos;
    QByteArray                  idExtra;
};

class GenericSqlMultiTableAssemblyAdapter : public GenericSqlAssemblyAdapter {
public:
    GenericSqlMultiTableAssemblyAdapter(GenericSqlDbi* dbi, const U2DataId& assemblyId, const AssemblyCompressor* compressor, GenericSqlDbRef* ref, U2OpStatus& os);
    virtual ~GenericSqlMultiTableAssemblyAdapter();
    ...
    const QVector<GenericSqlMtaSingleTableAdapter*>& getAdapters() const;
    const QVector<QByteArray>& getIdExtrasPerRange() const;

    GenericSqlDbRef* getDbRef() const;

    GenericSqlMtaSingleTableAdapter* getAdapterByRowAndElenRange(int rowRange, int elenRange, bool createIfNotExits, U2OpStatus& os);

protected:
    virtual GenericSqlMtaSingleTableAdapter* createAdapter(int rowRange, int elenRange, U2OpStatus& os) = 0;
    ...
    QVector<GenericSqlMtaSingleTableAdapter*>             adapters;

    /** outer dim -> prow, inner dim -> elen */
    QVector< QVector<GenericSqlMtaSingleTableAdapter*> >  adaptersGrid;
...
class GenericSqlReadTableMigrationData {
public:
    GenericSqlReadTableMigrationData();
    GenericSqlReadTableMigrationData(qint64 oldId, GenericSqlMtaSingleTableAdapter* oldT, int newP);

    qint64                          readId;
    GenericSqlMtaSingleTableAdapter*     oldTable;
    int                             newProw;
};

class GenericSqlMultiTablePackAlgorithmAdapter : public PackAlgorithmAdapter {
public:
    GenericSqlMultiTablePackAlgorithmAdapter(GenericSqlMultiTableAssemblyAdapter* a);
    ~GenericSqlMultiTablePackAlgorithmAdapter();
...
private:
    ...
    void migrate(GenericSqlMtaSingleTableAdapter* newA, const QVector<GenericSqlReadTableMigrationData>& data, qint64 migratedBefore, qint64 totalMigrationCount, U2OpStatus& os);

    GenericSqlMultiTableAssemblyAdapter*                                         multiTableAdapter;
    QVector<GenericSqlSingleTablePackAlgorithmAdapter*>                          packAdapters;
    QVector< QVector<GenericSqlSingleTablePackAlgorithmAdapter*> >               packAdaptersGrid;
    QHash<GenericSqlMtaSingleTableAdapter*, QVector<GenericSqlReadTableMigrationData> >    migrations;

    static const int MAX_PERCENT_TO_REINDEX = 20;
};

// Class that multiplexes multiple read iterators into 1
class GenericSqlMtaReadsIterator : public U2DbiIterator<U2AssemblyRead> {
public:
    GenericSqlMtaReadsIterator(QVector< U2DbiIterator<U2AssemblyRead>* >& iterators, const QVector<QByteArray>& idExtras, bool sortedHint);

    virtual ~GenericSqlMtaReadsIterator();
...
class GenericSqlMTAPackAlgorithmDataIterator : public U2DbiIterator<PackAlgorithmData> {
public:
    GenericSqlMTAPackAlgorithmDataIterator(QVector< U2DbiIterator<PackAlgorithmData>* >& iterators, const QVector<QByteArray>& idExtras);

    virtual ~GenericSqlMTAPackAlgorithmDataIterator();
...
#endif // _U2_GENERIC_SQL_ASSEMBLY_MULTI_TABLE_DBI_H_
