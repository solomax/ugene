...
#include "util/GenericSqlModificationAction.h"

namespace U2 {

GenericSqlSequenceDbi::GenericSqlSequenceDbi(GenericSqlDbi* dbi) :
    U2SequenceDbi(dbi),
    GenericSqlChildDbiCommon(dbi)
{
}

GenericSqlSequenceDbi::~GenericSqlSequenceDbi() {}

U2Sequence GenericSqlSequenceDbi::getSequenceObject(const U2DataId& sequenceId, U2OpStatus& os) {
    GenericSqlTransaction t(db, os);
    Q_UNUSED(t);

    U2Sequence res;
    DBI_TYPE_CHECK(sequenceId, U2Type::Sequence, os, res);
    dbi->getObjectDbi()->getObject(res, sequenceId, os);
    CHECK_OP(os, res);
...
QByteArray GenericSqlSequenceDbi::getSequenceData(const U2DataId& sequenceId, const U2Region& region, U2OpStatus& os) {
    try {
        QByteArray res;
...
#ifdef UGENE_X86
        os.setError(QObject::tr("UGENE ran out of memory during the sequence processing. "
...
void GenericSqlSequenceDbi::createSequenceObject(U2Sequence& sequence, const QString& folder, U2OpStatus& os, U2DbiObjectRank rank) {
    GenericSqlTransaction t(db, os);
    Q_UNUSED(t);

    dbi->getSqlObjectDbi()->createObject(sequence, folder, rank, os);
    CHECK_OP(os, );
...
void GenericSqlSequenceDbi::updateSequenceObject(U2Sequence& sequence, U2OpStatus& os) {
    GenericSqlTransaction t(db, os);
    Q_UNUSED(t);
    ...
    dbi->getSqlObjectDbi()->updateObject(sequence, os);
    CHECK_OP(os, );

    GenericSqlObjectDbi::incrementVersion(sequence.id, db, os);
}
...
void GenericSqlSequenceDbi::updateSequenceData(const U2DataId& sequenceId, const U2Region& regionToReplace, const QByteArray& dataToInsert, const QVariantMap &hints, U2OpStatus& os) {
    GenericSqlTransaction t(db, os);
    Q_UNUSED(t);

    GenericSqlModificationAction updateAction(dbi, sequenceId);
...
void GenericSqlSequenceDbi::updateSequenceData(GenericSqlModificationAction& updateAction, const U2DataId& sequenceId, const U2Region& regionToReplace,
...
void GenericSqlSequenceDbi::undo(const U2DataId& seqId, qint64 modType, const QByteArray& modDetails, U2OpStatus& os) {
...
void GenericSqlSequenceDbi::redo(const U2DataId& seqId, qint64 modType, const QByteArray& modDetails, U2OpStatus& os) {
    if (U2ModType::sequenceUpdatedData == modType) {
...
/* Core methods */
/************************************************************************/
void GenericSqlSequenceDbi::updateSequenceDataCore(const U2DataId& sequenceId, const U2Region& regionToReplace, const QByteArray& dataToInsert, const QVariantMap &hints, U2OpStatus& os) {
...
/* Undo/redo methods */
/************************************************************************/
void GenericSqlSequenceDbi::undoUpdateSequenceData(const U2DataId& sequenceId, const QByteArray& modDetails, U2OpStatus& os) {
...
void GenericSqlSequenceDbi::redoUpdateSequenceData(const U2DataId& sequenceId, const QByteArray& modDetails, U2OpStatus& os) {
    U2Region replacedRegion;
    U2Region replacedByRegion;
    QByteArray oldData;
    QByteArray newData;
